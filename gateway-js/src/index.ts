...
export class ApolloGateway implements GraphQLService {
  ...
  private queryPlannerPointer?: RefCounter<QueryPlannerPointer>;
  ...
  private loadStatic(config: StaticGatewayConfig) {
    ...
    this.queryPlannerPointer = new RefCounter(
      getQueryPlanner(composedSdl),
      dropQueryPlanner,
    );
  ...
  public executor = <TContext>(
    ...
    return this.queryPlannerPointer!.withBorrow((pointer) =>
      this._executor(requestContext, pointer),
    );
  };
  ...
  private _executor = async <TContext>(
    ...
    if (this.experimental_didResolveQueryPlan) {
      this.experimental_didResolveQueryPlan({
        ...
        serviceMap,
        requestContext,
        // NOTE: This is the only place where the raw QueryPlannerPointer
        // `queryPlannerPointer` can escape. The callback method
        // didResolveQueryPlan should not assume that the pointer is valid.
        operationContext,
      });
    }
